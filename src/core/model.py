# FILE: src/core/model.py (Definitive, Final, Fully-Featured, and Optimized Version)
import numpy as np
import random
...
class GillespieSimulation:
    ...
    def _precompute_env_params(self, **kwargs):
        ...
        if env_def and isinstance(env_def, dict):
            if env_def.get("scrambled"):
                patches = env_def.get("patches", [{"id": 0, "params": {}}])
        ...
        if event in self.event_to_idx:
            ...
            if self.free_indices:
                ...
                if idx >= self.MAX_EVENTS:
                    raise MemoryError(
                        f"SummedRateTree capacity ({self.MAX_EVENTS}) exceeded."
                    )
                self.next_event_idx += 1
            self.event_to_idx[event] = idx
            self.idx_to_event[idx] = event
        self.tree.update(idx, rate)
        self.cell_to_events[parent_cell].add(event)
    ...
    def _remove_event(self, event: Tuple):
        ...
        if not self.cell_to_events[parent_cell]:
            del self.cell_to_events[parent_cell]
    ...
    def _update_single_cell_events(self, h: Hex):
        ...
        if empty_neighbors:
            ...
            for neighbor in empty_neighbors:
                b_wt, b_m = self._get_params_for_q(neighbor.q)
    ...
    def _update_cell_and_neighbors(self, h: Hex):
        neighbors = self._get_neighbors_periodic(h)
        self._update_single_cell_events(h)
    ...
    def step(self):
        if (
            self.step_count > 0
            and self.step_count % self.cache_prune_step_interval == 0
        ):
    ...
    def _axial_to_cube(self, q: int, r_offset: int) -> Hex:
        return Hex(
            q, r_offset - (q + (q & 1)) // 2, -q - (r_offset - (q + (q & 1)) // 2)
        )
    ...
    def _prune_neighbor_cache(self):
        ...
        for h in [h for h in self.neighbor_cache if h.q < prune_q]:
            del self.neighbor_cache[h]

    def _get_params_for_q(self, q: int) -> Tuple[float, float]:
        q_idx = max(0, min(q, self.length - 1))
        return self.patch_params[self.q_to_patch_index[q_idx]]
