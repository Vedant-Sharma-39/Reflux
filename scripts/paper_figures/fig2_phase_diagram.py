# FILE: scripts/paper_figures/fig2_phase_diagram.py (Definitive Version)
...
def calculate_susceptibility(sorted_pivot_table):
    """
    Calculates susceptibility assuming the input pivot table is already sorted by index.
    """
    # The gradient is calculated along axis=0 (down the rows).
    sus_values = np.abs(np.gradient(sorted_pivot_table.values, axis=0))
    return pd.DataFrame(sus_values, index=sorted_pivot_table.index, columns=sorted_pivot_table.columns)
...
def main():
    ...
    output_path = os.path.join(project_root, "data", args.campaign_id, "analysis", "figure2_phase_diagram_publication.png")
    ...
    print(f"Curated phi values for plotting: {plot_phis}")
    ...
    for i, phi in enumerate(plot_phis):
        ...
        # --- THE CRITICAL FIX ---
        # Create the pivot table AND immediately sort it by the index (s).
        # This ensures all downstream operations use correctly ordered data.
        rho_pivot = df_slice.pivot_table(index="s", columns="log10_k_total", values="avg_rho_M").sort_index(ascending=False)
        # --- END FIX ---
        ...
        sns.heatmap(sus_pivot, ax=ax_sus, cmap="inferno", norm=SymLogNorm(linthresh=min_pos_sus, vmin=0, base=10), cbar_kws={"label": r"Susceptibility, $\chi$"})
    ...
    # The ax.invert_yaxis() call is no longer needed because we sorted the data beforehand.
    
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print(f"\nFigure 2 (Publication Version) saved to {output_path}")
