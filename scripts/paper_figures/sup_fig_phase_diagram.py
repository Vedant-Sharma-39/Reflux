...
def main():
    ...
    output_path = os.path.join(
        ...
        args.campaign_id,
        "analysis",
        "sup_fig_phase_diagram_details.png",
    )
    ...
    if df.empty:
        ...
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, "Phase Diagram: No Data Available", ha="center", va="center")
    ...
    plot_phis = sorted(
        list(
            set(
                [
                    min(phi_slices_available, key=lambda x: abs(x - ip))
                    for ip in ideal_phis
                ]
            )
        )
    )
    ...
    fig.suptitle(
        "Supplementary Figure: Phase Diagram of Mutant Invasion", fontsize=28, y=1.05
    )
    ...
    for i, phi in enumerate(plot_phis):
        ...
        ax_rho, ax_sus = axes[0, i], axes[1, i]
        ...
        sns.heatmap(
            ...
            vmin=0,
            vmax=1.0,
            cbar_kws={"label": r"$\langle\rho_M\rangle$"},
        )
        ax_rho.set_title(rf"$\phi = {phi:.2f}$", fontsize=18)

        min_pos_sus = (
            sus_pivot[sus_pivot > 0].min().min()
            if not sus_pivot[sus_pivot > 0].isnull().all().all()
            else 1e-9
        )
        sns.heatmap(
            sus_pivot,
            ...
            cbar_kws={"label": r"Susceptibility, $\chi$"},
        )
    ...
    print(f"\nPhase Diagram Figure saved to {output_path}")
