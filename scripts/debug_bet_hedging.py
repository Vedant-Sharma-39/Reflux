...
def main():
    # --- NEW: Command-line argument for environment selection ---
    ...
    parser.add_argument(
        ...
        default="symmetric",
        help="Type of patch environment to test: 'symmetric' (35x35) or 'asymmetric' (90x30).",
    )
    ...
    # Map the command-line choice to a specific environment definition from config.py
    ENV_CHOICES = {
        "symmetric": "debug_viz_strong",
        "asymmetric": "asymmetric_90_30_strong",
    }
    selected_env_name = ENV_CHOICES[args.env]
    # --- END NEW ---

    try:
        # Use the base parameters from the default debug experiment
        exp_config = EXPERIMENTS["debug_bet_hedging"]
        params = exp_config["sim_sets"]["main"]["base_params"].copy()

        # --- MODIFICATION: Override the environment with our selected one ---
    ...
    # Make the output filename unique to the test being run
    ...
    with tqdm(total=max_steps, desc=f"Simulating ({args.env})") as pbar:
        ...
        snapshot_path = snapshot_dir / f"snap_00.png"
