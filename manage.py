...
@app.command()
def launch(
    experiment_name: Optional[str] = typer.Argument(None, help="Experiment to launch, e.g., 'phase_diagram'"),
    yes: bool = typer.Option(False, "--yes", "-y", help="Automatically answer yes to all confirmation prompts.")
):
    """Syncs config.py with the master task list and submits missing jobs, batching if necessary."""
    if not experiment_name:
        typer.echo("Please choose an experiment to launch:"); experiment_name = _interactive_select(VALID_EXP_NAMES)
    if experiment_name not in VALID_EXP_NAMES:
        typer.secho(f"Error: Invalid experiment name '{experiment_name}'.", fg=typer.colors.RED); raise typer.Exit(1)
        
    config = EXPERIMENTS[experiment_name]
    campaign_id = config["campaign_id"]
    typer.secho(f"🚀 Managing Campaign: {experiment_name} ({campaign_id})", fg=typer.colors.CYAN)

    newly_added_count = _update_master_task_list(experiment_name)
    if newly_added_count > 0:
        typer.secho(f"Found and added {newly_added_count} new tasks to the master list.", fg=typer.colors.GREEN)
    else:
    ...
    raw_data_dir = data_dir / "raw"; raw_data_dir.mkdir(exist_ok=True)
    ...
    if summary_file.exists():
        try:
            ...
            if 'task_id' in df_summary.columns: completed_ids = set(df_summary['task_id'].astype(str))
        except (pd.errors.EmptyDataError, FileNotFoundError): pass
    ...
    with open(master_task_file) as fin, open(missing_tasks_file, "w") as fout:
        for line in fin:
            if json.loads(line).get("task_id") not in completed_ids: fout.write(line); missing_count += 1
    ...
    if missing_count == 0:
        typer.secho("\n🎉 All tasks are complete.", fg=typer.colors.GREEN); raise typer.Exit()
    ...
    if num_batches > 1: prompt_msg = f"\nCampaign is too large. Submit in {num_batches} separate job array batches?"
    if not yes and not typer.confirm(prompt_msg): raise typer.Abort()
...
def status(experiment_name: Optional[str] = typer.Argument(None)):
    if not experiment_name: experiment_name = _interactive_select(VALID_EXP_NAMES)
    if experiment_name not in VALID_EXP_NAMES: typer.secho(f"Error: Invalid experiment name.", fg=typer.colors.RED); raise typer.Exit(1)
    config = EXPERIMENTS[experiment_name]; campaign_id = config["campaign_id"]
    typer.secho(f"📊 Status for Campaign: {experiment_name} ({campaign_id})", fg=typer.colors.CYAN)
    data_dir = PROJECT_ROOT / "data" / campaign_id
    master_task_file = data_dir / f"{campaign_id}_master_tasks.jsonl"
    raw_dir = data_dir / "raw"
    summary_file = data_dir / "analysis" / f"{campaign_id}_summary_aggregated.csv"
    if not master_task_file.exists():
        typer.secho("  [!] Master task list not found. Run 'launch' first.", fg=typer.colors.YELLOW); raise typer.Exit()
    if raw_dir.exists() and any(raw_dir.glob("chunk_*.jsonl")):
        typer.secho("\n[!] Found unconsolidated raw data.", fg=typer.colors.YELLOW)
        typer.echo("      Run consolidation and wait for it to finish before checking status."); typer.secho(f"      python3 manage.py consolidate {campaign_id}", bold=True); raise typer.Exit()
    completed_ids = set()
    if summary_file.exists():
        try:
            df_summary = pd.read_csv(summary_file, low_memory=False)
    ...
    total_tasks = sum(1 for _ in open(master_task_file))
    completed_count = len(completed_ids); missing_count = total_tasks - completed_count
    ...
    typer.secho(f"  Progress:  {completed_count} / {total_tasks} ({percentage:.2f}%)", fg=color, bold=True)
    typer.secho(f"  Completed: {completed_count}", fg=typer.colors.GREEN)
    typer.secho(f"  Missing:   {missing_count}", fg=typer.colors.RED)
    if missing_count == 0: typer.secho("\n🎉 Campaign is 100% complete!", fg=typer.colors.GREEN, bold=True)
    else: typer.echo("\n   To run missing jobs, use 'launch':"); typer.secho(f"   python3 manage.py launch {experiment_name}", bold=True)

@app.command()
def consolidate(campaign_id: Optional[str] = typer.Argument(None), cpus: int = typer.Option(8, "--cpus", "-c"), time: str = typer.Option("00:30:00", "--time", "-t")):
    if not campaign_id:
        typer.echo("Please choose a campaign to consolidate:")
        all_campaigns = sorted(list(set([v['campaign_id'] for v in EXPERIMENTS.values()])))
    ...
    typer.secho(f"Submitting parallel consolidation job for '{campaign_id}'...", fg=typer.colors.BLUE)
    log_dir = PROJECT_ROOT / "slurm_logs"
    worker_script = PROJECT_ROOT / "scripts" / "utils" / "parallel_consolidate_worker.py"
    sbatch_cmd = ["sbatch", "--parsable", "--job-name", job_name, "--output", str(log_dir / f"{job_name}_%j.log"), "--cpus-per-task", str(cpus), "--mem", "16G", "--time", time, "--wrap", f"python3 {worker_script} {campaign_id} {PROJECT_ROOT}"]
    result = subprocess.run(sbatch_cmd, capture_output=True, text=True)
    if result.returncode != 0:
        typer.secho(f"sbatch submission failed:\n{result.stderr}", fg=typer.colors.RED); raise typer.Exit(1)
    job_id = result.stdout.strip()
    typer.secho(f"✅ Consolidation job submitted! Job ID: {job_id}", fg=typer.colors.GREEN)
    typer.secho(f"  Monitor with: squeue -j {job_id}", bold=True)

@app.command(name="plot")
def plot_figure(figure_name: Optional[str] = typer.Argument(None)):
    if not figure_name: figure_name = _interactive_select(VALID_FIG_NAMES + ["all"])
    figures_to_run = VALID_FIG_NAMES if figure_name == 'all' else [figure_name]
    for fig in figures_to_run:
        if fig not in FIG_MAP: continue
        ...
        if result.returncode != 0:
            typer.secho(f"Error generating {fig}. Plotting script failed.", fg=typer.colors.RED)
            # --- THE FIX: USE SECHO for bold, not ECHO ---
            typer.secho("--- STDERR from plotting script ---\n" + (result.stderr or "No stderr output."), bold=True)
            raise typer.Exit(1)
    typer.secho("\n✅ All requested figures generated.", fg=typer.colors.GREEN)

# ... (debug, _update_master_task_list, helpers, etc. are correct and unchanged from previous answers)
# ... I will include them here to be absolutely certain the file is complete.
...
def _submit_sbatch_job(job_name, hpc_params, array_range, task_file, raw_data_dir):
    ...
    sbatch_cmd = ["sbatch", "--parsable", "--job-name", job_name, "--array", array_range, "--output", str(log_dir / f"{job_name}_%A_%a.log"), "--mem", hpc_params.get("mem", "2G"), "--time", hpc_params.get("time", "01:00:00")]
    if hpc_params.get("partition"): sbatch_cmd.extend(["--partition", hpc_params["partition"]])
    if hpc_params.get("ntasks"): sbatch_cmd.extend(["--ntasks", str(hpc_params["ntasks"])])
    if hpc_params.get("cpus_per_task"): sbatch_cmd.extend(["--cpus-per-task", str(hpc_params["cpus_per_task"])])
    sbatch_cmd.extend([str(PROJECT_ROOT / "scripts" / "run_chunk.sh"), str(PROJECT_ROOT), str(log_dir), str(task_file), str(raw_data_dir), str(hpc_params.get("sims_per_task", 50))])
    result = subprocess.run(sbatch_cmd, capture_output=True, text=True)
    if result.returncode != 0:
        typer.secho(f"sbatch command failed for batch '{job_name}':\n{result.stderr}", fg=typer.colors.RED); raise SbatchSubmissionError()
    else:
        typer.secho(f"✅ Batch '{job_name}' submitted successfully! ID: {result.stdout.strip()}", fg=typer.colors.GREEN)
...
def clean(experiment_name: Optional[str] = typer.Argument(None)):
    ...
    typer.secho(f"This will DELETE data and logs for '{campaign_id}':", fg=typer.colors.RED, bold=True)
    if not typer.confirm("\nAre you absolutely sure?"): raise typer.Abort()
    if data_dir.exists(): shutil.rmtree(data_dir); typer.echo(f"Removed {data_dir}")
    if log_dir_campaign.exists(): shutil.rmtree(log_dir_campaign); typer.echo(f"Removed {log_dir_campaign}")
    typer.secho("✨ Cleanup complete.", fg=typer.colors.GREEN)

@app.command()
def debug(experiment_name: Optional[str] = typer.Argument(None), line: Optional[int] = typer.Option(None, "--line", "-l")):
    if not experiment_name: experiment_name = _interactive_select(VALID_EXP_NAMES)
    if not line: line = typer.prompt("Which task line number to debug?", type=int)
    campaign_id = EXPERIMENTS[experiment_name]["campaign_id"]
    master_task_file = PROJECT_ROOT / "data" / campaign_id / f"{campaign_id}_master_tasks.jsonl"
    if not master_task_file.exists(): _update_master_task_list(experiment_name)
    try:
        with open(master_task_file) as f: params_json = f.readlines()[line - 1]
    except IndexError: typer.secho(f"Error: Line {line} is out of bounds.", fg=typer.colors.RED); raise typer.Exit(1)
    params = json.loads(params_json); task_id = params["task_id"]
    typer.secho(f"--- 🕵️  Debugging Task {task_id} (Line {line}) ---", bold=True); typer.echo(json.dumps(params, indent=2))
    output_dir = PROJECT_ROOT / "data" / campaign_id / "raw"; output_dir.mkdir(exist_ok=True)
    final_output_path = output_dir / f"{task_id}.json"
    worker_script = str(PROJECT_ROOT / "src" / "worker.py")
    cmd = [sys.executable, worker_script, "--params", params_json, "--output-dir", str(output_dir)]
    env = os.environ.copy(); env["PROJECT_ROOT"] = str(PROJECT_ROOT)
    typer.echo("\n--- Running Worker ---")
    proc = subprocess.run(cmd, capture_output=True, text=True, env=env)
    if proc.returncode == 0:
        try:
            result_data = json.loads(proc.stdout)
            with open(final_output_path, "w") as f: json.dump(result_data, f, indent=2)
            typer.secho("\n✅ Success", fg=typer.colors.GREEN); typer.echo(f"Output file created: {final_output_path}")
        except json.JSONDecodeError:
            typer.secho("\n❌ FAILED: Worker output was not valid JSON.", fg=typer.colors.RED)
    else:
        typer.secho(f"\n❌ FAILED", fg=typer.colors.RED); typer.secho("--- Worker STDERR ---\n" + (proc.stderr or "No stderr output."), bold=True)

def _update_master_task_list(experiment_name: str) -> int:
    config = EXPERIMENTS[experiment_name]; campaign_id = config["campaign_id"]
    data_dir = PROJECT_ROOT / "data" / campaign_id; data_dir.mkdir(parents=True, exist_ok=True)
    master_task_file = data_dir / f"{campaign_id}_master_tasks.jsonl"
    ...
    if master_task_file.exists():
        with open(master_task_file, "r") as f:
            for line in f:
                try: existing_task_ids.add(json.loads(line)['task_id'])
                except: continue
    new_task_ids = set(desired_task_map.keys()) - existing_task_ids
    if not new_task_ids: return 0
...
def _consolidate_and_ensure_file_exists(campaign_id: str):
    ...
    if timeseries_raw_dir.exists():
        ...
        if raw_ts_files:
            typer.secho(f"Moving {len(raw_ts_files)} new timeseries files...", fg=typer.colors.BLUE)
...
def _is_job_running(job_name_prefix: str) -> bool:
    ...
    for job in running_jobs:
        if job.strip().startswith(job_name_prefix): return True
    return False

def _interactive_select(options: list) -> str:
    for i, option in enumerate(options, 1): typer.echo(f"  [{i}] {option}")
    choice = typer.prompt("Your choice?", type=int)
    if 1 <= choice <= len(options): return options[choice - 1]
    typer.secho("Invalid choice.", fg=typer.colors.RED); raise typer.Abort()
